# Maintainer: Robert Kuszinger
# Source/Original: Maciej Sieczka <msieczka at sieczka dot org>
...
conflicts=('grass72')
source=('https://grass.osgeo.org/grass72/source/snapshot/grass-7.2.svn_src_snapshot_latest.tar.gz')
md5sums=('SKIP')
noextract=()
...
prepare() {
  msg "mimic version"
  if [ -h "${srcdir}/grass-${pkgver}" ]
  then
    echo "Unlink existing build dir "
    unlink "${srcdir}/grass-${pkgver}"
  fi
  ln -s `ls -d ${srcdir}/grass*_201*` "${srcdir}/grass-${pkgver}"
  ...
  sed -i -e "s,^Name=GRASS GIS 7\$,Name=GRASS GIS ${pkgver}," \
         ...
         -e "s,^Icon=grass72\$,Icon=/usr/share/icons/${pkgname}-64x64.png," \
...
build() {
  ...
  FORTIFY_FLAGS=`echo "$CPPFLAGS" | sed '/^.*\(-D_FORTIFY_SOURCE=.\).*$/s//\1/'` CPPFLAGS=`echo "$CPPFLAGS" | sed 's/-D_FORTIFY_SOURCE=.//g'` CFLAGS="$FORTIFY_FLAGS $CFLAGS" CXXFLAGS="-std=c++98 $FORTIFY_FLAGS $CXXFLAGS" ./configure \
    --prefix=/opt \
